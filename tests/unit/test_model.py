...
def test_owner_field_should_be_a_valid_form_of_email():
    issue = NewIssue(
        ...
        created_at=str(datetime.now(timezone.utc)),
        status=Status.TO_DO.value,
        responsible_expert='owner@email.com',
    )
    assert issue.responsible_expert == 'owner@email.com'


@pytest.mark.parametrize('responsible_expert_value', ['', 'dummy owner email'])
def test_test_owner_field_must_have_an_email_in_valid_format(responsible_expert_value):
    with pytest.raises(ValueError) as error:
        NewIssue(
            username='dummy name',
            category=Defect.PERIPHERAL.value,
...
def test_owner_field_should_be_created_empty_when_not_provided():
    ...
    assert issue.responsible_expert == ''
